...
class RenameModal(discord.ui.Modal, title="Rename VC"):
    ...
    async def on_submit(self, interaction):
        ...
        if vc: await vc.edit(name=str(self.name.value)); await interaction.response.send_message("✅ VC renamed.",ephemeral=True)
...
class LimitModal(discord.ui.Modal, title="Set VC limit"):
    ...
    async def on_submit(self,interaction):
        vc=await self.cog.owned_vc(interaction)
        if not vc: return
        try: n=int(str(self.limit.value)); assert 0<=n<=99
        except: return await interaction.response.send_message("❌ Enter a number from 0 to 99.",ephemeral=True)
...
class TempVoice(commands.Cog):
    ...
    async def setup_cmd(self,ctx):
        ...
        cat=await ctx.guild.create_category("TempVoice"); join=await ctx.guild.create_voice_channel("➕ Join to Create",category=cat)
        msg=await ctx.channel.send("## 🎙️ TempVoice Control Panel\nJoin **➕ Join to Create**. Then use these buttons to control the temporary VC you own.",view=TempVoicePanel(self))
        d={"join_id":join.id,"category_id":cat.id,"panel_channel_id":ctx.channel.id,"panel_message_id":msg.id}; self.setup[ctx.guild.id]=d; await self.save_setup(ctx.guild.id,d)
        await ctx.send(f"✅ Configured! Join {join.mention} to create your VC. Panel: {msg.jump_url}")
    ...
    async def panel_cmd(self,ctx):
        ...
        await ctx.send("## 🎙️ TempVoice Control Panel\nControls apply to the temporary VC you currently own.",view=TempVoicePanel(self))
