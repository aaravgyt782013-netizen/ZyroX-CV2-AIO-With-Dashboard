...
class RenameModal(discord.ui.Modal, title="Rename VC"):
    ...
    async def on_submit(self, interaction):
        ...
        if vc:
            await vc.edit(name=str(self.name.value)); await interaction.response.send_message("✏️ VC renamed.",ephemeral=True)
...
class LimitModal(discord.ui.Modal, title="Set VC limit"):
    ...
    async def on_submit(self,interaction):
        ...
        await vc.edit(user_limit=n); await interaction.response.send_message(f"👥 Limit set to **{n or 'unlimited'}**.",ephemeral=True)

class TransferSelect(discord.ui.View):
    def __init__(self,cog,interaction):
        super().__init__(timeout=120); self.cog=cog; self.owner=interaction.user
        s=discord.ui.UserSelect(placeholder="Select the new VC owner",min_values=1,max_values=1)
        s.callback=self.select; self.add_item(s)
    async def select(self,interaction):
        ...
        await self.cog.set_owner(vc.id,member.id); await vc.set_permissions(member,manage_channels=True,move_members=True,connect=True)
...
class PermitSelect(discord.ui.View):
    def __init__(self,cog,interaction):
        super().__init__(timeout=120); self.cog=cog; self.owner=interaction.user
        s=discord.ui.UserSelect(placeholder="Select a member to permit",min_values=1,max_values=1)
        s.callback=self.select; self.add_item(s)
    async def select(self,interaction):
...
class CategorySelect(discord.ui.View):
    def __init__(self,cog,ctx):
        ...
        opts=[discord.SelectOption(label=c.name[:100],value=str(c.id),description="Create Join to Create here") for c in ctx.guild.categories[:25]]
        s=discord.ui.Select(placeholder="Select a server category",options=opts,min_values=1,max_values=1)
        s.callback=self.select;self.add_item(s)
    async def select(self,i):
        cat=i.guild.get_channel(int(self.children[0].values[0]))
...
class VoiceSelect(discord.ui.View):
    def __init__(self,cog,ctx):
        super().__init__(timeout=300);self.cog=cog;self.ctx=ctx
        opts=[discord.SelectOption(label=c.name[:100],value=str(c.id),description=f"Category: {c.category.name[:80]}" if c.category else "No category") for c in ctx.guild.voice_channels[:25]]
        s=discord.ui.Select(placeholder="Select an existing voice channel",options=opts,min_values=1,max_values=1)
        s.callback=self.select;self.add_item(s)
    async def select(self,i):
        ch=i.guild.get_channel(int(self.children[0].values[0]))
...
class TextSelect(discord.ui.View):
    def __init__(self,cog,ctx,cat,existing=None):
        ...
        me=ctx.guild.me
        channels=[c for c in ctx.guild.text_channels if not me or c.permissions_for(me).send_messages]
        opts=[discord.SelectOption(label=c.name[:100],value=str(c.id),description="TempVoice interface channel") for c in channels[:25]]
        s=discord.ui.Select(placeholder="Select the interface text channel",options=opts,min_values=1,max_values=1)
        s.callback=self.choose;self.add_item(s)
...
class TempVoice(commands.Cog):
    def __init__(self,bot):self.bot=bot;self.setup={};self.channels={};bot.loop.create_task(self.load())
    async def load(self):
    ...
    async def on_voice_state_update(self,member,before,after):
        ...
        if after.channel and after.channel.id==d["join_id"]:
            ...
            if cat:
                ...
                await self.save_channel(vc.id,member.guild.id,member.id);await vc.set_permissions(member,manage_channels=True,move_members=True,connect=True);await member.move_to(vc)
