...
async def _announce(self, ctx, title: str, description: str, channel: discord.TextChannel, ping: str = "none"):
    ...
    if ping_text.lower() in {"@everyone", "everyone"}:
        ...
        if role is not None:
            ...
            return await ctx.reply(
                f"{CROSS} Invalid ping. Use `none`, `@everyone`, `@here`, or a role mention/ID.",
    ...
    embed.set_author(name=f"{ctx.guild.name} • Announcement", icon_url=ctx.guild.icon.url if ctx.guild.icon else discord.Embed.Empty)
    embed.set_footer(
    ...
    confirm = discord.Embed(
        title=f"{TICK} Announcement Sent",
        description=f"Your announcement was sent to {channel.mention}.",
        color=0xFF0000,
    )
    if content:
