...
import discord
from utils.emoji import ARROWRED, HEART3, INDEX, ZYROXLINKS
...
class MentionSelectView(LayoutView):
    def __init__(self, message, bot, prefix):
        ...
        self.select = Select(placeholder=f"Start With {BotName}", options=[
            ...
            discord.SelectOption(label="Help", emoji="❓", description="Show the help menu", value="help"),
            discord.SelectOption(label="Links", emoji=ZYROXLINKS, description="Useful bot links", value="links"),
        ])
        self.select.callback = self.on_select
        self.add_item(self._container(f"**{message.guild.name}**", self._home_content()))

    def _home_content(self):
        return f"> {HEART3} **Hey {self.message.author.mention}**\n> {ARROWRED} **Prefix For This Server: `{self.prefix}`**\n\n___Type `{self.prefix}help` for more information.___"

    def _container(self, title, content):
        return Container(TextDisplay(title), Separator(visible=True), TextDisplay(content), ActionRow(self.select))
    ...
    async def on_select(self, interaction):
        ...
        if selected == "home":
            ...
            content = f"## ❓ Help Menu\nUse **`{self.prefix}help`** to open the full interactive command help menu.\n\nTempVoice: **`{self.prefix}tempvoice setup`**\nPrefix: **`{self.prefix}prefix set <new_prefix>`**"
        else:
            content = f"**[Invite {BotName}](https://discord.com/oauth2/authorize?client_id=1516313619476779120&permissions=8&integration_type=0&scope=bot)**\n**[Join Support Server]({serverLink})**"
        self.clear_items()
